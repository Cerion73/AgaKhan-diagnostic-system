...
class PractitionerViewset(ModelViewSet):
    ...
    def otp_request(self, request):
        ...
        return Response({'error': serializer.errors}, status=status.HTTP_400_BAD_REQUEST, template_name='otp_request.html')
    ...
    def otp_verify(self, request):
        ...
        try:
            ...
            user.save()
            return Response({'message': 'OTP verified successfully.'}, status=status.HTTP_200_OK, template_name='dashboard.html')

        except CustomUser.DoesNotExist:
            return Response({'error': 'Invalid OTP or phone number.'}, status=status.HTTP_400_BAD_REQUEST, template_name='otp_verify.html')

    @action(methods=['get', 'post'], detail=False, url_name='sign-up', url_path='sign-up.html', renderer_classes = [TemplateHTMLRenderer])
    def signup(self, request):
        serializer = PractitionerSerializer(data=request.data)

        if not serializer.is_valid():
            return Response({'errors': serializer.errors}, status=status.HTTP_400_BAD_REQUEST, template_name='signup.html')
